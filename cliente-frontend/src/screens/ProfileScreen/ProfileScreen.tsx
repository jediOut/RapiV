...
export default function ProfileScreen({ navigation, onLogout }: Props) {
  ...
  const loadUserData = async () => {
    const [userData, ratingHistory] = await Promise.all([
      sessionStorage.getUser(),
      ratingApi.getMyRatings().catch(() => []),
    ]);
    setUser(userData);
              ...
              {ratings.length ? (
                ratings.slice(0, 6).map((rating) => (
                  <View key={rating.id} style={styles.ratingItem}>
                    <View>
                      <Text style={styles.ratingTarget}>
                        {rating.targetType === 'BUSINESS' ? 'Comercio' : 'Repartidor'}
                      </Text>
